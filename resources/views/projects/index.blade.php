                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <div class="mt-4">
                        <form action="{{ url('projects', $project->id) }}/comments" method="POST">
                            ...
                            <button type="submit" class="bg-indigo-500 text-white px-4 py-2 rounded-md "  style="border: 1px solid blue;margin-left: auto; background-color: #1E90FF; display: flex; justify-content: flex-end;">Submit</button>
                        </form>
                    </div>
                    ...
                    <div class="mt-6">
                        ...
                        <ul class="mt-4">
                            @foreach ($project->comments as $comment)
                                <li class="mb-4">
                                    created by:<span class=" font-semibold" style="color: #ADD8E6"> {{ $comment->user->name }}</span>
                                     <p class="mt-2 text-gray-600 dark:text-gray-400" style="margin-left: 20px;">{{ $comment->content }}</p>
                                        ...
                                        <div  style="margin-left: auto; display: flex;" >
                                            @if ($comment->user_id === auth()->user()->id)
                                                <form id="edit-form-{{ $comment->id }}" action="{{ route('comments.update', $comment->id) }}" method="POST" style="display: none;">
                                                    ...
                                                    @method('PUT')
                                                    <div class="flex">
                                                    <input name="content" value="{{ $comment->content }}" rows="3" class="px-2 py-3 p-3 border border-gray-300 dark:border-gray-700 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-900 dark:text-gray-300">
                                                        <button type="submit" class="bg-yellow-500 text-white px-4 py-2 rounded-md mr-2 hover:bg-yellow-700" style="background-color: #ffc107!important; padding: 12px; margin-left: 5px;">
                                                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pen" viewBox="0 0 16 16">
                                                                <path d="m13.498.795.149-.149a1.207 1.207 0 1 1 1.707 1.708l-.149.148a1.5 1.5 0 0 1-.059 2.059L4.854 14.854a.5.5 0 0 1-.233.131l-4 1a.5.5 0 0 1-.606-.606l1-4a.5.5 0 0 1 .131-.232l9.642-9.642a.5.5 0 0 0-.642.056L6.854 4.854a.5.5 0 1 1-.708-.708L9.44.854A1.5 1.5 0 0 1 11.5.796a1.5 1.5 0 0 1 1.998-.001zm-.644.766a.5.5 0 0 0-.707 0L1.95 11.756l-.764 3.057 3.057-.764L14.44 3.854a.5.5 0 0 0 0-.708l-1.585-1.585z"/>
                                                            </svg>
                                                        </button>
                                                        <button type="button" id="cancel-button-{{ $comment->id }}" onclick="event.preventDefault(); cancelEdit('{{ $comment->id }}');" class="bg-gray-500 text-white px-4 py-2 rounded-md hover:bg-gray-700" style="padding: 12px; margin-left: 5px;">
                                                ...
                                                <button type="button" id="edit-button-{{ $comment->id }}" onclick="event.preventDefault(); editComment('{{ $comment->id }}');" class="text-white rounded-md ml-2" style="background-color: #ffc107!important; padding: 12px;">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pen" viewBox="0 0 16 16">
                                                        <path d="m13.498.795.149-.149a1.207 1.207 0 1 1 1.707 1.708l-.149.148a1.5 1.5 0 0 1-.059 2.059L4.854 14.854a.5.5 0 0 1-.233.131l-4 1a.5.5 0 0 1-.606-.606l1-4a.5.5 0 0 1 .131-.232l9.642-9.642a.5.5 0 0 0-.642.056L6.854 4.854a.5.5 0 1 1-.708-.708L9.44.854A1.5 1.5 0 0 1 11.5.796a1.5 1.5 0 0 1 1.998-.001zm-.644.766a.5.5 0 0 0-.707 0L1.95 11.756l-.764 3.057 3.057-.764L14.44 3.854a.5.5 0 0 0 0-.708l-1.585-1.585z"/>
                                                    </svg>
                                                </button>
                                                <form action="{{ route('comments.destroy', $comment->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this comment?')">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="bg-red-500 text-white px-3 py-3 rounded-md ml-2 hover:bg-red-700" style="padding: 12px;">
